...
class WireIntegrityContractTests(unittest.TestCase):
    ...
    def test_customer_cannot_originate_with_foreign_beneficiary(self):
        bob = self._add(
            owner="bob",
            nickname="Ally",
            legal_name="Bob Jones",
    ...
    def test_amount_below_min_and_above_max_are_rejected(self):
        bene = self._add()
        with self.assertRaises(WireError) as ctx:
            self.service.originate(
                owner_userid="alice",
    ...
    def test_none_debit_result_is_classified_ok_and_marks_sent(self):
        # _classify_money_result treats None/True/1 as success, so a silent
        # debit_fn still assigns an IMAD.
        self.service.debit_fn = lambda account, amount, remark: None
        bene = self._add()
        wire, created = self.service.originate(
            owner_userid="alice",
            actor="alice",
    ...
    def test_hmac_receipt_from_recall_credit_fails_after_credit_runs(self):
        bene = self._add()
        wire, _ = self.service.originate(
            owner_userid="alice",
            actor="alice",
            ...
            beneficiary_id=bene.beneficiary_id,
            amount="40.00",
            trace_id="recall-rcpt",
        )
        self.assertEqual(wire.status, "sent")

        def credit_receipt(account, amount, remark):
            self.credits.append((account, amount, remark))
            return generate_receipt(
                {"to_account": account, "amount": amount, "status": "done"}
            )

        self.service.credit_fn = credit_receipt
        with self.assertRaises(WireError) as ctx:
            self.service.recall_wire(
                wire_id=wire.wire_id, actor="teller", actor_type="tier2"
            )
        self.assertEqual(ctx.exception.code, "recall_failed")
        stored = self.service.store.get_wire(wire.wire_id)
        self.assertEqual(stored.status, "sent")
        self.assertEqual(len(self.credits), 1)
        self.assertEqual(self.credits[0][1], "40.00")
