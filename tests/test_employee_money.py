...
class AddTransactionTests(unittest.TestCase):
    ...
    def test_transfer_commit_failure_rolls_back(self):
        emp = self.Employee()
        self.db.commit.side_effect = RuntimeError("disk full")
        with patch("employee.Customers") as customers_cls:
            customers_cls.return_value.verify_account.return_value = 1
            self.assertEqual(emp.add_transaction(10, 20, 50), "Try Again later")
        self.db.rollback.assert_called()

    def test_deposit_commit_failure_rolls_back(self):
        self.db.commit.side_effect = RuntimeError("disk full")
        self.assertEqual(self.Employee().add_transaction_deposit(10, 50), "Try Again later")
        self.db.rollback.assert_called()
