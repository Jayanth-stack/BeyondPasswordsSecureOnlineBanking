"""Per-account credit-limit policy.

Replaces the hardcoded $5000 ceiling in Customers.fund_transfers /
debit_request. Distinct from:

- Card PIN/lock (auth on the plastic)
- Account freeze (outbound lock of any account type)
- Velocity (rolling-window dollar/count)

Credit accounts use a signed balance: negative = utilized, positive =
overpayment/bonus. Available credit is:

    effective_limit + balance - open_reservations + over_buffer

Checking/savings are never gated. Deposits and payments onto the card skip.
Employees are still limited — this is a product ceiling, not an auth factor.
...
class CreditLimitPolicy:
    default_limit: Decimal = Decimal('5000.00')
    min_limit: Decimal = Decimal('500.00')
    max_limit: Decimal = Decimal('50000.00')
    over_buffer: Decimal = Decimal('0.00')
    request_max_increment: Decimal = Decimal('10000.00')
    max_pending_requests: int = 3
    reservation_ttl_seconds: int = 86400
    temp_increase_max_seconds: int = 90 * 24 * 3600
    max_temp_increase: Decimal = Decimal('10000.00')

    @classmethod
    def from_env(cls) -> 'CreditLimitPolicy':
        return cls(
            default_limit=_env_money('CREDIT_LIMIT_DEFAULT', '5000.00'),
            min_limit=_env_money('CREDIT_LIMIT_MIN', '500.00'),
            max_limit=_env_money('CREDIT_LIMIT_MAX', '50000.00'),
            over_buffer=_env_money('CREDIT_LIMIT_OVER_BUFFER', '0.00'),
            request_max_increment=_env_money('CREDIT_LIMIT_REQUEST_MAX', '10000.00'),
            max_pending_requests=_env_int('CREDIT_LIMIT_MAX_PENDING', 3),
            reservation_ttl_seconds=_env_int('CREDIT_LIMIT_RESERVE_TTL', 86400),
            temp_increase_max_seconds=_env_int('CREDIT_LIMIT_TEMP_MAX_SECONDS', 90 * 24 * 3600),
            max_temp_increase=_env_money('CREDIT_LIMIT_TEMP_MAX', '10000.00'),
        )
...
class LimitDecision:
    ...
    facility: Optional['CreditFacility'] = None
...
class CreditFacility:
    ...
    def to_dict(self, *, now: Optional[float] = None, balance: Optional[Decimal] = None,
                reserved: Optional[Decimal] = None, over_buffer: Decimal = Decimal('0.00')) -> Dict[str, Any]:
        ...
        if balance is not None:
            ...
            available = limit + balance - held + over_buffer
        temp_active = bool(self.temp_increase > 0 and self.temp_expires_at and self.temp_expires_at > clock)
        return {
            ...
            'reserved': money_str(reserved or Decimal('0.00')),
...
class MemoryCreditLimitStore:
    def __init__(self) -> None:
        self._lock = threading.Lock()
        self._facilities: Dict[str, CreditFacility] = {}
        self._by_account: Dict[str, str] = {}
        self._requests: Dict[str, LimitRequest] = {}
        self._reservations: Dict[str, Reservation] = {}

    def put_facility(self, facility: CreditFacility) -> None:
        with self._lock:
            self._facilities[facility.facility_id] = facility
            self._by_account[facility.account] = facility.facility_id

    def get_facility(self, facility_id: str) -> Optional[CreditFacility]:
        with self._lock:
            return self._facilities.get(facility_id)

    def get_by_account(self, account: str) -> Optional[CreditFacility]:
        with self._lock:
    ...
    def update_facility(self, facility: CreditFacility) -> None:
        self.put_facility(facility)

    def list_facilities(self, userid: Optional[str] = None) -> List[CreditFacility]:
        with self._lock:
    ...
    def put_request(self, item: LimitRequest) -> None:
        with self._lock:
            self._requests[item.request_id] = item

    def get_request(self, request_id: str) -> Optional[LimitRequest]:
        with self._lock:
            return self._requests.get(request_id)

    def update_request(self, item: LimitRequest) -> None:
        self.put_request(item)

    def list_requests(self, userid: Optional[str] = None, status: Optional[str] = None) -> List[LimitRequest]:
        with self._lock:
...
class SqliteCreditLimitStore:
    ...
    def _init(self) -> None:
        with self._lock, self._connect() as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_facilities_user ON facilities(userid)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_requests_user ON limit_requests(userid, status)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_reservations_acct ON reservations(account, status)')
            conn.commit()
        ...
        with self._lock, self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO facilities (
                    facility_id, userid, account, base_limit, temp_increase,
        ...
        with self._lock, self._connect() as conn:
            conn.execute(
                ...
                UPDATE facilities SET
                    userid=?, account=?, base_limit=?, temp_increase=?,
                ...
                (
                    ...
                    facility.temp_reason, facility.updated_at, facility.updated_by,
                    facility.updated_by_type, facility.facility_id,
                ),
            )
            conn.commit()
    ...
    def get_request(self, request_id: str) -> Optional[LimitRequest]:
        with self._lock, self._connect() as conn:
            row = conn.execute(
                'SELECT * FROM limit_requests WHERE request_id = ?', (request_id,)
            ).fetchone()
    ...
    def list_requests(self, userid: Optional[str] = None, status: Optional[str] = None) -> List[LimitRequest]:
        ...
        sql = 'SELECT * FROM limit_requests'
        if clauses:
    ...
    @staticmethod
    def _facility_row(facility: CreditFacility) -> Tuple[Any, ...]:
        return (
            facility.facility_id, facility.userid, facility.account,
            money_str(facility.base_limit), money_str(facility.temp_increase),
            facility.temp_expires_at, facility.temp_reason, facility.created_at,
            facility.updated_at, facility.updated_by, facility.updated_by_type,
        )

    @staticmethod
    def _facility_from_row(row: sqlite3.Row) -> CreditFacility:
        return CreditFacility(
            facility_id=row['facility_id'],
            userid=row['userid'],
            account=row['account'],
            base_limit=Decimal(row['base_limit']),
            temp_increase=Decimal(row['temp_increase'] or '0.00'),
    ...
    @staticmethod
    def _request_from_row(row: sqlite3.Row) -> LimitRequest:
        return LimitRequest(
...
class CreditLimitService:
    def __init__(
        self,
        policy: Optional[CreditLimitPolicy] = None,
        ...
        self.policy = policy or CreditLimitPolicy()
        self.store = store or MemoryCreditLimitStore()
        self.clock = clock
    ...
    def _assert_ownership(
        ...
        *,
        ...
        if account not in _own_account_set(own_accounts):
            raise CreditLimitError('limit_forbidden', 'Credit facility does not belong to this customer.')

    def _assert_staff(self, actor_type: str) -> None:
        if not self._actor_is_employee(actor_type):
            raise CreditLimitError('limit_forbidden', 'Only staff can change a credit limit.')
    ...
    def ensure_facility(self, *, userid: str, account: Any) -> CreditFacility:
    ...
    def _validate_limit(self, amount: Decimal) -> Decimal:
    ...
    def set_limit(
        ...
        *,
        ...
        amount = self._validate_limit(parse_money(limit))
    ...
    def adjust_limit(
        ...
        *,
        ...
        return self.set_limit(
            ...
            account=account_text,
            limit=facility.base_limit + change,
        ...
        *,
        ...
        if bump > self.policy.max_temp_increase:
            raise CreditLimitError(
                ...
                'Temporary increase exceeds the allowed maximum.',
        ...
        if ttl <= 0 or ttl > self.policy.temp_increase_max_seconds:
            raise CreditLimitError('invalid_ttl', 'Temporary increase duration is invalid.')
        facility = self.ensure_facility(userid=owner_userid, account=account_text)
        now = float(self.clock())
        combined = facility.effective_limit(now) + bump
        self._validate_limit(facility.base_limit)
        if combined > self.policy.max_limit:
            raise CreditLimitError(
                'limit_out_of_range',
                'Temporary increase would exceed the maximum credit line.',
                max=money_str(self.policy.max_limit),
            )
    ...
    def request_increase(
        ...
        *,
        ...
        if self._actor_is_employee(actor_type):
            raise CreditLimitError('limit_forbidden', 'Staff should set the limit directly.')
        account_text = normalize_account(account)
        self._assert_ownership(account=account_text, actor_type=actor_type, own_accounts=own_accounts)
        ...
        if target <= current:
            raise CreditLimitError('limit_not_increase', 'Requested limit must be higher than the current limit.')
        if target - facility.base_limit > self.policy.request_max_increment:
            raise CreditLimitError(
                'request_too_large',
                'Requested increase exceeds the allowed increment.',
        ...
        if len(pending) >= self.policy.max_pending_requests:
            raise CreditLimitError('request_limit', 'Too many pending credit-limit requests.')
        for item in pending:
            if item.requested_limit == target:
                raise CreditLimitError('request_duplicate', 'A matching increase request is already pending.')
        req = LimitRequest(
    ...
    def decide_request(
        ...
        *,
        ...
        if req is None:
            raise CreditLimitError('request_not_found', 'Credit-limit request was not found.')
        if req.status != 'pending':
            raise CreditLimitError('request_not_pending', 'Credit-limit request is no longer pending.')
        now = float(self.clock())
        req.decided_at = now
    ...
    def available_credit(
        ...
        *,
        ...
        facility = self.store.get_by_account(normalize_account(account))
        if facility is None:
            limit = self.policy.default_limit
        else:
            limit = facility.effective_limit(float(self.clock()))
        held = self.open_reserved(normalize_account(account))
        return limit + parse_balance(balance) - held + self.policy.over_buffer
    ...
    def evaluate(
        ...
        *,
        ...
        if op in INBOUND_OPERATIONS or op not in CHARGE_OPERATIONS:
            return LimitDecision(action='allow', reason='not_gated')
        if account in (None, ''):
            return LimitDecision(action='allow', reason='no_account')
        try:
            account_text = normalize_account(account)
        except AccountError:
            return LimitDecision(action='block', reason='invalid_account')
        ...
        is_credit = kind == 'credit' or self.is_credit_account(account_text, userid)
        if not is_credit:
            return LimitDecision(action='allow', reason='not_credit')

        if balance is None:
            balance = loaded_balance
        if balance is None:
            # Unknown utilization — leave the existing money handler to decide.
            return LimitDecision(action='allow', reason='balance_unknown')

        try:
            charge = parse_money(amount)
            bal = parse_balance(balance)
        except AmountError:
            return LimitDecision(action='block', reason='invalid_amount')

        facility = self.store.get_by_account(account_text)
        now = float(self.clock())
        limit = facility.effective_limit(now) if facility is not None else self.policy.default_limit
        reserved = self.open_reserved(account_text)
        available = limit + bal - reserved + self.policy.over_buffer
        if charge > available:
    ...
    def reserve(
        ...
        *,
        ...
        decision = self.evaluate(
            operation=operation if operation != 'transfer' else 'transfer',
            account=account,
            amount=amount,
            balance=balance,
        ...
        if decision.reason in {'not_gated', 'not_credit', 'no_account', 'balance_unknown'}:
            return None
        if decision.blocked:
            raise CreditLimitError(
                decision.reason,
                'Charge would exceed the available credit.',
        ...
        account_text = normalize_account(account)
        self.ensure_facility(userid=owner_userid, account=account_text)
        now = float(self.clock())
        item = Reservation(
    ...
    def snapshot(
        ...
        for facility in self.store.list_facilities(userid=userid):
            ...
            facilities.append(facility.to_dict(
                ...
                reserved=reserved,
                over_buffer=self.policy.over_buffer,
            ))
        ...
        return {
            'policy': {
                ...
                'over_buffer': money_str(self.policy.over_buffer),
            },
...
_DEFAULT_SERVICE: Optional[CreditLimitService] = None


def default_store() -> Any:
    path = os.environ.get('CREDIT_LIMIT_STORE') or os.path.join('SystemLogs', 'credit_limit.sqlite')
    return SqliteCreditLimitStore(path)


def build_service(
    is_credit_loader=None,
    balance_loader=None,
    store=None,
    policy=None,
) -> CreditLimitService:
    return CreditLimitService(
        policy or CreditLimitPolicy.from_env(),
...
def set_service(service: Optional[CreditLimitService]) -> None:
    global _DEFAULT_SERVICE
    _DEFAULT_SERVICE = service


def get_service() -> CreditLimitService:
    global _DEFAULT_SERVICE
...
def charge_allowed(
    ...
    amount: Any,
    balance: Any,
    account_type: Any = 'credit',
) -> bool:
    """Used by the money handler. True if the charge may proceed.

    Falls back to the historical $5000 ceiling if the service cannot run, so
    existing credit-card behaviour is preserved when the store is unavailable.
    """
    kind = str(account_type or '').strip().lower()
    if kind != 'credit':
        try:
            return parse_balance(balance) >= parse_money(amount)
        except (AmountError, Exception):
            return False
    try:
        service = get_service()
        ...
        try:
            return parse_balance(balance) - parse_money(amount) >= -LEGACY_LIMIT
        except Exception:
            return False


def _blocked_payload(decision: LimitDecision, operation: str) -> Dict[str, Any]:
    messages = {
        'credit_limit_exceeded': 'Charge would exceed the available credit.',
        'invalid_amount': 'Enter a valid amount.',
        'invalid_account': 'Invalid account.',
    }
    payload = {
        'message': messages.get(decision.reason, 'Credit-limit check blocked this charge.'),
    ...
    *,
    ...
    if reserve:
        try:
            ...
            status = 403 if exc.code == 'credit_limit_exceeded' else 400
...
def _error_body(exc: CreditLimitError) -> Dict[str, Any]:
...
def handle_set_limit(service: CreditLimitService, own_accounts_loader=None):
    ...
    return jsonify({
        ...
        'facility': facility.to_dict(now=float(service.clock())),
        'CreditLimits': service.snapshot(owner),
    }), 200


def handle_adjust_limit(service: CreditLimitService, own_accounts_loader=None):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
...
def handle_temp_increase(service: CreditLimitService, own_accounts_loader=None):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
    ...
    try:
        ...
        return jsonify(_error_body(exc)), _error_status(exc.code)
    return jsonify({
        'message': 'Temporary increase granted',
        'facility': facility.to_dict(now=float(service.clock())),
        'CreditLimits': service.snapshot(owner),
    }), 200


def handle_revoke_temp(service: CreditLimitService, own_accounts_loader=None):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
    ...
    try:
        ...
        return jsonify(_error_body(exc)), _error_status(exc.code)
    return jsonify({
        'message': 'Temporary increase revoked',
        'facility': facility.to_dict(now=float(service.clock())),
        'CreditLimits': service.snapshot(owner),
    }), 200


def handle_request_increase(service: CreditLimitService, own_accounts_loader=None):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
    ...
    try:
        ...
        return jsonify(_error_body(exc)), _error_status(exc.code)
    return jsonify({
        'message': 'Credit-limit increase requested',
        'request': req.to_dict(),
        'CreditLimits': service.snapshot(owner),
    }), 201


def handle_decide_request(service: CreditLimitService):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
    ...
    try:
        ...
        return jsonify(_error_body(exc)), _error_status(exc.code)
    return jsonify({
        'message': 'Credit-limit request ' + req.status,
        'request': req.to_dict(),
        'CreditLimits': service.snapshot(req.userid),
    }), 200


def handle_list_limits(service: CreditLimitService):
    userid, error = _require_session_user()
    if error:
        return error
    values = request.get_json(silent=True) or {}
    ...
    return jsonify({'CreditLimits': service.snapshot(owner)}), 200


def attach_credit_limit_routes(app, service: CreditLimitService, own_accounts_loader=None) -> None:
    @app.route('/setCreditLimit', methods=['POST', 'GET'])
    def set_credit_limit_route():
        return handle_set_limit(service, own_accounts_loader=own_accounts_loader)

    @app.route('/adjustCreditLimit', methods=['POST', 'GET'])
    def adjust_credit_limit_route():
        return handle_adjust_limit(service, own_accounts_loader=own_accounts_loader)

    @app.route('/grantTempCreditIncrease', methods=['POST', 'GET'])
    def grant_temp_credit_route():
        return handle_temp_increase(service, own_accounts_loader=own_accounts_loader)

    @app.route('/revokeTempCreditIncrease', methods=['POST', 'GET'])
    def revoke_temp_credit_route():
        return handle_revoke_temp(service, own_accounts_loader=own_accounts_loader)

    @app.route('/requestCreditLimitIncrease', methods=['POST', 'GET'])
    def request_credit_limit_route():
        return handle_request_increase(service, own_accounts_loader=own_accounts_loader)

    @app.route('/decideCreditLimitRequest', methods=['POST', 'GET'])
    def decide_credit_limit_route():
        return handle_decide_request(service)

    @app.route('/listCreditLimits', methods=['POST', 'GET'])
    def list_credit_limits_route():
        return handle_list_limits(service)
