...
function fillUkPay(data) {
  var snapshot = data.UkPay || {};
  var summary = document.getElementById('ukpay_summary');
  if (summary) {
    summary.innerHTML = 'Active: ' + (snapshot.active_count || 0) +
      ' &middot; open: ' + (snapshot.open_count || 0) +
      ' &middot; YTD sent: $' + (snapshot.ytd_sent || '0.00') +
  ...
  var clockEl = document.getElementById('ukpay_clock');
  if (clockEl && snapshot.clock) {
    clockEl.innerHTML = 'CHAPS cutoff ' + (snapshot.clock.cutoff || '17:00') +
      ' &middot; value date ' + (snapshot.clock.value_date || '--') +
      ' &middot; GBPUSD ' + (snapshot.fx_rate || '1.2500') +
      (snapshot.clock.after_cutoff ? ' &middot; after cutoff' : '');
  }
  fillLinkedAccountSelect('ukpay_default_account');
  var table = document.getElementById('ukpay_bene_tbl');
  if (!table) {
    return;
  }
  var body = table.getElementsByTagName('tbody')[0];
  body.innerHTML = '';
  var selection = document.getElementById('ukpay_bene_select');
  selection.options.length = 1;
  var benes = snapshot.beneficiaries || [];
  for (var i = 0; i < benes.length; i++) {
    var row = body.insertRow(-1);
  ...
  var hist = document.getElementById('ukpay_history_tbl');
  var histBody = hist.getElementsByTagName('tbody')[0];
  histBody.innerHTML = '';
  var openSelect = document.getElementById('ukpay_open_select');
  openSelect.options.length = 1;
  ...
  for (var j = 0; j < pays.length; j++) {
    ...
    prow.insertCell(2).innerHTML = '&pound;' + pays[j].amount_gbp;
    prow.insertCell(3).innerHTML = '$' + pays[j].debit_usd;
    prow.insertCell(4).innerHTML = pays[j].status;
    if (!pays[j].cancelable) {
      continue;
    }
    var openOpt = document.createElement('OPTION');
    openOpt.value = pays[j].payment_id;
    openOpt.innerHTML = pays[j].nickname + ' &pound;' + pays[j].amount_gbp + ' (' + pays[j].status + ')';
    openSelect.options.add(openOpt);
  }
}

function postUkPay(path, payload, okMsg) {
  ...
  fetch(homeURL + path, {
    ...
    return response.json().then(function(body) {
      var result = document.getElementById('ukpay_result');
      if (!response.ok) {
        if (result) { result.innerHTML = (body && (body.message || body.error)) || 'Failed'; }
        window.alert((body && (body.message || body.error)) || 'Request failed');
        if (body && body.UkPay) {
          fillUkPay({ UkPay: body.UkPay });
        }
        return;
      }
      if (result) {
        if (body && body.preview) {
          result.innerHTML = 'GBP ' + body.preview.amount_gbp +
            ' &middot; debit $' + body.preview.debit_usd +
...
$(document).ready(function() {
  ...
  var specialElementHandlers = {
    ...
    $('#account_details_btn').on('click', function(){
      ...
      $('#ukpay_menu').css('background-color','maroon');
    });
    ...
    $('#ukpay_menu').on('click', function(){
      if($('#ukpay_pane').css('display')=='none'){
          $('#ukpay_pane').show().siblings('div').hide();
      }
      $('#ukpay_menu').css('background-color','#FF6600');
      $('#my_accounts_menu').css('background-color','maroon');
      $('#service_requests_menu').css('background-color','maroon');
      $('#pending_transaction_requests_menu').css('background-color','maroon');
      $('#linked_accounts_menu').css('background-color','maroon');
    ...
    $('#ukpay_add_btn').on('click', function(){
      if($('#ukpay_nickname').val() == '' || $('#ukpay_legal_name').val() == '' || $('#ukpay_default_account').val() == 'select'){
        window.alert('Nickname, legal name, and internal account are required.');
        return;
      }
      postUkPay('addUkPayBeneficiary', {
        nickname: $('#ukpay_nickname').val(),
        legal_name: $('#ukpay_legal_name').val(),
        sort_code: $('#ukpay_sort_code').val(),
        account_number: $('#ukpay_account_number').val(),
        iban: $('#ukpay_iban').val(),
        city: $('#ukpay_city').val(),
        postcode: $('#ukpay_postcode').val(),
        default_account: $('#ukpay_default_account').val()
      }, 'Beneficiary added');
    });
    $('#ukpay_preview_btn').on('click', function(){
      if($('#ukpay_bene_select').val() == 'select' || $('#ukpay_amount').val() == ''){
        window.alert('Select a beneficiary and amount.');
        return;
      }
      postUkPay('previewUkPay', {
        beneficiary_id: $('#ukpay_bene_select').val(),
        amount: $('#ukpay_amount').val(),
        scheme: $('#ukpay_scheme').val(),
        account: $('#ukpay_default_account').val() == 'select' ? null : $('#ukpay_default_account').val()
      }, 'Preview');
    });
    $('#ukpay_send_btn').on('click', function(){
      if($('#ukpay_bene_select').val() == 'select' || $('#ukpay_amount').val() == ''){
        window.alert('Select a beneficiary and amount.');
        return;
      }
      postUkPay('sendUkPay', {
        ...
        amount: $('#ukpay_amount').val(),
        scheme: $('#ukpay_scheme').val(),
        purpose: $('#ukpay_purpose').val(),
        memo: $('#ukpay_memo').val(),
        account: $('#ukpay_default_account').val() == 'select' ? null : $('#ukpay_default_account').val()
      }, 'UK payment originated');
    });
    $('#ukpay_cancel_btn').on('click', function(){
      if($('#ukpay_open_select').val() == 'select'){ window.alert('Select an open payment.'); return; }
      postUkPay('cancelUkPay', { payment_id: $('#ukpay_open_select').val() }, 'Cancelled');
    });
    $('#ukpay_pause_btn').on('click', function(){
      if($('#ukpay_bene_select').val() == 'select'){ window.alert('Select a beneficiary.'); return; }
      postUkPay('pauseUkPayBeneficiary', { beneficiary_id: $('#ukpay_bene_select').val() }, 'Paused');
    });
    $('#ukpay_resume_btn').on('click', function(){
      if($('#ukpay_bene_select').val() == 'select'){ window.alert('Select a beneficiary.'); return; }
      postUkPay('resumeUkPayBeneficiary', { beneficiary_id: $('#ukpay_bene_select').val() }, 'Resumed');
    });
    $('#ukpay_archive_btn').on('click', function(){
      if($('#ukpay_bene_select').val() == 'select'){ window.alert('Select a beneficiary.'); return; }
      postUkPay('archiveUkPayBeneficiary', { beneficiary_id: $('#ukpay_bene_select').val() }, 'Archived');
    });
