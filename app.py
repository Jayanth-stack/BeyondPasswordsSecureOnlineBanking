...
from utility.sdd import (
    attach_sdd_routes,
    build_service as build_sdd_service,
    get_service as get_sdd_service,
    set_service as set_sdd_service,
)
...
def get_customer_data():
    ...
    try:
        response = {
            ...
            'Sdd': _sdd_snapshot(customer_id, actor=customer_id, actor_type='customer'),
        }
        return jsonify(response), 200
...
def _sdd_snapshot(userid, actor=None, actor_type='customer'):
    service = get_sdd_service()
    if service is None:
        return {'enabled': False, 'debtors': [], 'mandates': [], 'collections': []}
    try:
        return service.snapshot(userid, actor=actor, actor_type=actor_type)
    except Exception:
        return {'enabled': False, 'debtors': [], 'mandates': [], 'collections': []}
...
sdd_service = build_sdd_service(
    debit_fn=_link_debit,
    credit_fn=_link_credit,
    accounts_fn=_link_accounts,
)
set_sdd_service(sdd_service)
attach_sdd_routes(app, sdd_service)
