...
from utility.sepa import (
    attach_sepa_routes,
    build_service as build_sepa_service,
    get_service as get_sepa_service,
    set_service as set_sepa_service,
)
...
def get_customer_data():
    ...
    try:
        response = {
            ...
            'Sepa': _sepa_snapshot(customer_id, actor=customer_id, actor_type='customer'),
        }
        return jsonify(response), 200
...
def _sepa_snapshot(userid, actor=None, actor_type='customer'):
    service = get_sepa_service()
    if service is None:
        return {'enabled': False, 'creditors': [], 'transfers': []}
    try:
        return service.snapshot(userid, actor=actor, actor_type=actor_type)
    except Exception:
        return {'enabled': False, 'creditors': [], 'transfers': []}
...
sepa_service = build_sepa_service(
    debit_fn=_link_debit,
    credit_fn=_link_credit,
    accounts_fn=_link_accounts,
)
set_sepa_service(sepa_service)
attach_sepa_routes(app, sepa_service)
